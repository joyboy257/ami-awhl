...
export async function getKeywordsData(params: QueryParams): Promise<KeywordsDTO> {
  ...
  const keywords = await query<{
    ...
    SELECT 
      ...
      (
        SELECT d.domain FROM wellness.serp_results sr
        JOIN wellness.serp_snapshots ss ON sr.snapshot_id = ss.id
        JOIN wellness.domains d ON sr.domain_id = d.id
        WHERE ss.query_id = sq.id AND sr.rank_position = 1
        ORDER BY ss.captured_at DESC
        LIMIT 1
      ) as top_domain_url,
      (
        SELECT sr.rank_position FROM wellness.serp_results sr
        JOIN wellness.serp_snapshots ss ON sr.snapshot_id = ss.id
        WHERE ss.query_id = sq.id
        ORDER BY ss.captured_at DESC, sr.rank_position ASC
        LIMIT 1
      ) as top_rank
    FROM wellness.search_queries sq
    JOIN wellness.verticals v ON sq.vertical_id = v.id
  ...
  return {
    keywords: keywords.map((k) => ({
      ...
      top_clinic_name: k.top_domain_url,
      top_clinic_rank: k.top_rank,
    })),
    totalCount,
