...
export async function getMarketMapData(params: QueryParams): Promise<MarketMapDTO> {
  ...
  // Add search filter for clinic name or domain
  if (search && search.trim()) {
    conditions.push(`(c.name ILIKE $${paramIndex} OR d.domain ILIKE $${paramIndex})`);
  ...
  const columnMapping: Record<string, string> = {
    ...
    'serpVisibility': 'serp_visibility',
    'serp_visibility': 'serp_visibility',
    'seoHygiene': 'seo_hygiene',
    'seo_hygiene': 'seo_hygiene',
  };

  // Validate sort column to prevent SQL injection
  const mappedSort = columnMapping[sort] || 'competitor_score';
  ...
  // Get paginated data - use subquery to allow proper sorting after deduplication
  ...
  const competitors = await query<DBCompetitor>(`
    ...
    ORDER BY ${mappedSort} ${sortOrder} NULLS LAST
